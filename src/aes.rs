...
pub(crate) fn AES_ENCRYPT(block_buf: &mut [u8], key: &Block) {
    let mut encrypt_key = AesKey {
        rd_key: [0; RD_KEY_MAX_SIZE],
        rounds: 0,
    };

    aes_set_encrypt_key(key, 128, &mut encrypt_key);

    aes_encrypt(BlockBuf::InPlace(block_buf), &encrypt_key);
}

pub(crate) fn AES_DECRYPT(block_buf: &mut [u8], key: &Block) {
    let mut decrypt_key = AesKey {
        rd_key: [0; RD_KEY_MAX_SIZE],
        rounds: 0,
    };

    aes_set_decrypt_key(key, 128, &mut decrypt_key);

    aes_decrypt(block_buf, &decrypt_key);
}
